...
export function useCashLedger() {
  ...
  async function addTransaction(data) {
    const { data: inserted, error } = await supabase.from('cash_ledger').insert([{
  ...
  // Gross totals double-count settled amounts: e.g. a person we lent
  // 100k who later paid us back 50k (recorded as a "lent" repayment
  // reducing their debt) would have their card correctly show net 50k
  // receivable, but the old gross sum added 100k to "They Owe Us" AND
  // another 100k to "We Owe Them" for the borrow leg, wildly
  // overstating both cards. Net-first collapses each person to a
  // single direction before summing.
