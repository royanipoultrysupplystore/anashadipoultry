...
import { Banknote, Plus, ChevronRight, ArrowUpRight, ArrowDownLeft, Phone } from 'lucide-react'
...
export default function CashLedger() {
  ...
  const { persons, loading, totalLent, totalBorrowed, addTransaction, updateTransaction } = useCashLedger()
  ...
  const [settleUseStoreCash, setSettleUseStoreCash] = useState(true)
  ...
  async function handleSubmit(e) {
    ...
    if (ok) {
      if (!editTx) {
        ...
        if (useStoreCashFlag) await recordLedgerCash(form.type, amt, ok.id, form.person_name.trim(), form.transaction_date)
  ...
  async function handleSettle(e) {
    ...
    if (ok) {
      ...
      if (settleUseStoreCash) await recordLedgerCash(type, amt, ok.id, settleTarget.name, settleForm.transaction_date)
        ...
        <div className="space-y-3">
          {filtered.map(person => {
            ...
            return (
              <div
                key={person.name}
                onClick={() => navigate(`/cash-ledger/${encodeURIComponent(person.name.toLowerCase())}`)}
                className="bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3 p-4 cursor-pointer hover:bg-slate-50 transition-colors"
              >
                ...
                {net !== 0 && (
                  <button
                    ...
                    className={`shrink-0 flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg text-xs font-medium ${
                      net > 0
                        ? 'bg-green-600 text-white hover:bg-green-700'
                        : 'bg-red-600 text-white hover:bg-red-700'
                    }`}
                  >
                    {net > 0 ? <ArrowDownLeft size={14} /> : <ArrowUpRight size={14} />}
                    <span className="hidden sm:flex flex-col items-center leading-none">
                      <span>{net > 0 ? t('cashLedger.receivePayment') : t('cashLedger.makePayment')}</span>
                      <span dir="rtl" className="opacity-90 mt-0.5">{net > 0 ? 'ترلاسه کول' : 'ادایګي'}</span>
                    </span>
                  </button>
                )}
        ...
        {settleTarget && (
          <form onSubmit={handleSettle} className="space-y-4">
            ...
            <label className={`flex items-center gap-2 text-sm rounded-lg px-3 py-2 cursor-pointer border ${settleTarget.isReceiving ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
              <input type="checkbox" checked={settleUseStoreCash} onChange={e => setSettleUseStoreCash(e.target.checked)} className="rounded" />
              💵 {settleTarget.isReceiving ? t('storeCash.addToStoreCash') : t('storeCash.fromStoreCash')}
            </label>
            <div className="flex gap-3 justify-end pt-1">
