...
export default function MedicineSupplierDetail() {
  ...
  async function handlePayment(e) {
    ...
    if (ok) {
      ...
      // Store cash drawer is AFN — only record cash-out for AFN payments.
      if (isNew && paymentFromStoreCash && currency === 'AFN' && wasPaid > 0) {
    ...
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          else if (deleteTarget?.type === 'payment') { await deletePayment(deleteTarget.item.id); await removeByReference(deleteTarget.item.id) }
          else if (deleteTarget?.type === 'purchase') await deletePurchase(deleteTarget.item)
        }}
